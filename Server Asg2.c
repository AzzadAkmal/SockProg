...
{
	int socket_desc, client_sock, c, hostName, ka, option_len;
	int optval, optlen, optval1;
	struct sockaddr_in server, client;
	char client_message[2000], host[2000], clientname[2000], 
serverIP[50], clientIP[20], myMTU[20];
...
&client, (socklen_t *) &c);
	...
	inet_ntop(AF_INET, &server.sin_addr, serverIP, sizeof(serverIP));
	...
	if(strcasecmp(a,s) == 0)
	{
		strcpy(client_message, "Waalaikumussalam\n");		
	}
	...
	{
		strcpy(client_message, serverIP);
	}
	...
	else if(strcasecmp(a, "getmtu") == 0)
	{
		...
		sprintf(myMTU, "%d", ifr.ifr_mtu);
		strcpy(client_message, myMTU);
	}
...
SO_BROADCAST, (char *) &optval1, sizeof(int));
	...
	{
		optlen = sizeof(optval);
		ka = getsockopt(socket_desc, IPPROTO_TCP, TCP_NODELAY, 
(char *) &optval, &optlen);
		if(optval == 0)
...
SO_KEEPALIVE, (char *) &optval1, sizeof(int));
	...
	{
		strcpy(client_message, "Invalid input");
	}
	send(client_sock, client_message, 200, 0);
